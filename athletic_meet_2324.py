...
if grade == 1 or grade == 2 :
    ...
    st.write("###### 200m Time: *ss.ms OR m.ss.ms*")
    st.write("###### 600m/800m Time: *m.ss.ms*")
...
if grade == 1 or grade == 2 :
    ...
    st.write("### 600m/800m Running (Top 3)")
    table5 = merged_df[(merged_df["Unit"] == unit) & (merged_df["Std"] == select_grade) & (merged_df["Gender"] == gender)][["Name","Std","Sec","Unit","Gender","House","600m","600m(s)"]].nsmallest(3, '600m(s)', keep='all').dropna(subset=['600m(s)'])
    st.dataframe(table5, column_config={"600m(s)": None} ,use_container_width=True ,hide_index=True)
    st.write("### Shot Put (Top 3)")
    table6 = merged_df[(merged_df["Unit"] == unit) & (merged_df["Std"] == select_grade) & (merged_df["Gender"] == gender)][["Name","Std","Sec","Unit","Gender","House","Shot Put"]].nlargest(3, 'Shot Put', keep="all").dropna(subset=['Shot Put'])
    st.dataframe(table6, use_container_width=True ,hide_index=True)
    st.write("### Long Jump (Top 3)")
    table7 = merged_df[(merged_df["Unit"] == unit) & (merged_df["Std"] == select_grade) & (merged_df["Gender"] == gender)][["Name","Std","Sec","Unit","Gender","House","Long Jump"]].nlargest(3, 'Long Jump', keep="all").dropna(subset=['Long Jump'])
